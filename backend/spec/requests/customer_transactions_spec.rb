...
RSpec.describe "CustomerTransactions", type: :request do
  ...
  let!(:transaction) { Transaction.create!(customer_id: customer.id, user_id: user.id, payment_method: 'credit', amount: 100.0) }
  ...
  describe "POST /customers/:customer_id/transactions" do
    context "with valid params" do
      ...
      it "redirects to the created transaction" do
        post customer_transactions_path(customer_id: customer.id), params: { transaction: valid_attributes }
        expect(response).to redirect_to(customer_transaction_path(customer_id: customer.id, id: Transaction.last.id))
      end
    end
